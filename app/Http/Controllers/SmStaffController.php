...
class SmStaffController extends Controller
{
    ...
    public function staffStore(Request $request)
    {
        $request->validate([
            ...
            'mobile' => "required|max:11|min:11|unique:sm_staffs", //"^(?:+88|88)?(01[3-9]\d{8})$" 
        ...
        if ($request->role_id == 4) {
            ...
            $username = trim("T" . $request->mobile);
        }
        try {
            DB::beginTransaction();
            ...
            if ($request->role_id == 4) {
                ...
                $this->userAddToCentral($username, $url);
            }
            try {
                $user = new User();
    ...
    public function staffUpdate(Request $request)
    {
        ...
        $request->validate([
            ...
            'date_of_joining' => "required",
            'mobile' => 'required|max:11|min:11|unique:sm_staffs,mobile,' . $request->staff_id,
            'gender_id' => "required",
            'current_address' => "required|max:255",
            'basic_salary' => "required|max:100",
