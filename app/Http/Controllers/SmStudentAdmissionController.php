...
class SmStudentAdmissionController extends Controller
{
    ...
    function userAddToCentral($user, $url)
    {
        $curl = curl_init();

        curl_setopt_array($curl, array(
            CURLOPT_URL => 'https://demoscl.xyz/node1/api/insert-data',
            ...
            CURLOPT_POSTFIELDS => array('username' => $user, 'url' => $url),
        ));
    ...
    public function studentStore(Request $request)
    {
        ...
        if ("student" . $request->guardians_email != "") {
            $is_duplicate = SmStudent::where('school_id', Auth::user()->school_id)
                ->where('email', "student" . $request->guardians_email)
                ->first();
        ...
        try {
            ...
            $studet_users = $user_stu->username = trim("S" . $request->guardians_phone);
            $user = $user_stu->username;
            $user_stu->email = "student" . $request->guardians_email;
            $user_stu->password = Hash::make(123456);
            // $user_stu->password = Hash::make(trim("S".$request->guardians_phone));
            $user_stu->school_id = Auth::user()->school_id;
            $user_stu->created_at = $academic_year->year . '-01-01 12:00:00';

            $this->userAddToCentral($studet_users, $url);
            $user_stu->save();
            $user_stu->toArray();
            ...
            try {
                if ($request->parent_id == "") {
                    ...
                    if (!empty($request->guardians_email)) {
                        ...
                        $user_parent->username = trim("G" . $request->guardians_phone);
                    }
                    $user_parent->email = $request->guardians_email;
                    $user_parent->password = Hash::make(123456);
                    // $user_parent->password = Hash::make(trim("G".$request->guardians_phone)) ;
                    $user_parent->school_id = Auth::user()->school_id;
                    $user_parent->created_at = $academic_year->year . '-01-01 12:00:00';
                    $parent_users = trim("G" . $request->guardians_phone);

                    $this->userAddToCentral($parent_users, $url);
                ...
                try {
                    ...
                    try {
                        ...
                        $student->email = "";
                        $student->email = "student" . $request->guardians_email;
                        $student->mobile = $request->guardians_phone;
                        ...
                        if ($student) {
                            $user_info[] =  array('email' => "student" . $request->guardians_email, 'id' => $student->id, 'slug' => 'student');
                        }
    ...
    {
        $classs = SmClass::find($request->class);
        // dd($classs);
        $user = "";
        $user2 = "";
        $request->validate(
                ...
                if (!empty($data)) {
                    foreach ($data as $key => $value) {
                        ...
                        if ($ad_check->count() > 0) {
                            ...
                            StudentBulkTemporary::where('user_id', Auth::user()->id)->delete();
                            Toastr::error($ad_check->first_name . 'already have this admission number.', 'Failed');
                            return redirect()->back();
                        }
                        ...
                        if ($value->guardian_email != "") {
                            ...
                            if ($chk >= 1) {
                                DB::rollback();
                                StudentBulkTemporary::where('user_id', Auth::user()->id)->delete();
                                Toastr::error($value->guardian_email . 'already in use.', 'Failed');
                                return redirect()->back();
                            }
                        }

                        if ($value->guardian_phone != "") {
                            $chk =  DB::table('sm_parents')->where('guardians_mobile', $value->guardian_phone)->where('school_id', Auth::user()->school_id)->count();
                            if ($chk >= 1) {
                                DB::rollback();
                                StudentBulkTemporary::where('user_id', Auth::user()->id)->delete();
                                Toastr::error($value->guardian_phone . 'already available.', 'Failed');
                                return redirect()->back();
                            }
                        }
                        $student_usernam = "";
                        try {
                            $url =  'http://' . $_SERVER['HTTP_HOST'] . "/";
                            $student_username = trim("G" . $value->guardian_phone);
                            $this->userAddToCentral($student_username, $url);
                            $parent_username = trim("S" . $value->guardian_phone);
                            $this->userAddToCentral($parent_username, $url);
                        } catch (\Exception $e) {
                            DB::rollback();
                            StudentBulkTemporary::where('user_id', Auth::user()->id)->delete();
                            Toastr::error("failed for" . $student_usernam, 'Failed');
                            return redirect()->back();
                        }
                        ...
                        try {
                            ...
                            if (empty($value->guardian_phone)) {
                                $user_stu->username = $value->admission_number;
                                $user = trim("S" . $value->guardian_phone);
                            } else {
                                $user_stu->username = trim("S" . $value->guardian_phone);
                                $user = trim("S" . $value->guardian_phone);
                            }
                            // $url =  'http://' . $_SERVER['HTTP_HOST'] . "/";
                            // $student_username = trim("G" . $value->guardian_phone);
                            ...
                            // curl_setopt_array($curl, array(
                            // CURLOPT_URL => 'http://demoscl.xyz/schoolapi/user/add',
                            // CURLOPT_RETURNTRANSFER => true,
                            // CURLOPT_ENCODING => '',
                            // CURLOPT_MAXREDIRS => 10,
                            // CURLOPT_TIMEOUT => 0,
                            ...
                            // CURLOPT_HTTP_VERSION => CURL_HTTP_VERSION_1_1,
                            // CURLOPT_CUSTOMREQUEST => 'POST',
                            // CURLOPT_POSTFIELDS => 'user='.$user.'&url='.$url,
                            ...
                            try {
                                ...
                                if (empty($value->guardian_email)) {
                                    ...
                                    $user_parent->username  = trim("G" . $value->guardian_phone);
                                    $user2 = trim("G" . $value->guardian_phone);
                                } else {
                                ...
                                try {
                                    ...
                                    try {
                                        ...
                                        $student->academic_id = $request->session;
                                        //new two field added
                                        $student->monthly_fee = $classs->fee;
