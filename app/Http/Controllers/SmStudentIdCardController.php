...
class SmStudentIdCardController extends Controller
{
    ...
    public function generateadmitCard()
    {
        ...
        try {
            $id_cards = SmStudentIdCard::where('active_status', 1)->where('school_id', Auth::user()->school_id)->get();
            $classes = SmClass::where('active_status', 1)->where('academic_id', getAcademicId())->where('school_id', Auth::user()->school_id)->get();
            return view('backEnd.admin.generate_admit_card', compact('id_cards', 'classes'));
        } catch (\Exception $e) {
            Toastr::error('Operation Failed', 'Failed');
            return redirect()->back();
        }
    ...
    public function generateIdCardSearch(Request $request)
    {
        ...
        try {
            ...
            if (!$request->section)
                ...
                $students = SmStudent::with('className', 'parents', 'section', 'gender', 'school')->where('active_status', 1)
    ...
    public function generateadmitCardSearch(Request $request)
    {
        ...
        try {
            ...
            $classes = SmClass::where('active_status', 1)->where('academic_id', getAcademicId())->where('school_id', Auth::user()->school_id)->get();
            $id_cards = SmStudentIdCard::where('active_status', 1)->where('academic_id', getAcademicId())->where('school_id', Auth::user()->school_id)->get();
            return view('backEnd.admin.generate_admit_card', compact('id_cards', 'class_id', 'classes', 'students', 'card_id', 'section'));
        } catch (\Exception $e) {
            Toastr::error('Operation Failed', 'Failed');
            return redirect()->back();
        }
    ...
    public function generateIdCardPrint($s_id, $c_id)
    {
        ...
        try {
            ...
            foreach ($s_ids as $sId) {
                $students[] = SmStudent::with(['class:id,class_name', 'sections:section_name', 'school'])->find($sId);
            }
    ...
    public function generateAdmitCardPrint($s_id, $c_id)
    {
        // dd($s_id, $c_id);
        set_time_limit(2700);
        try {
            ...
            //dd($students[0]->section_id);

            $subject = SmExam::with('GetSubjectName:id,subject_name')->where('class_id', $c_id)->where('section_id', $students[0]->section_id)

                ->get();
            $examtype = SmExam::with('GetExamTitle:id,title')->where('class_id', $c_id)->where('section_id', $students[0]->section_id)
            ...
            if ($examtype) {;
                ...
                return view('backEnd.admin.student_admit_card_print', ['id_card' => $id_card, 'students' => $students, 'subject' => $subject, 'examtype' => $examtype]);

                $pdf = PDF::loadView('backEnd.admin.student_id_card_print_2', ['id_card' => $id_card, 'students' => $students]);
                return $pdf->stream($id_card->title . '.pdf');
            } else {
                Toastr::error('NO exam Setup yet', 'Failed');
                return redirect()->back();
            }
        } catch (\Exception $e) {
            Toastr::error('Operation Failed', 'Failed');
