...

{
    ...
    public function Featchfrommachine()
    {
        return $saved = Attendance::storeAttendance();
    }
    //end
    ...
    public function studentSearch(Request $request)
    {
        ...
        try {
            ...
            // dd($already_assigned_students);
            return view('backEnd.studentInformation.student_attendance', compact('classes', 'sections', 'date', 'class_id', 'section_id', 'already_assigned_students', 'new_students', 'attendance_type', 'search_info'));
        } catch (\Exception $e) {
            if (ApiBaseMethod::checkUrl($request->fullUrl())) {
                return ApiBaseMethod::sendError($e->getMessage(), 'error');
            }
            Toastr::error('Operation Failed', 'Failed');
            return redirect()->back();
        }
