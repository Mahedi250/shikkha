...
class SmApiController extends Controller
{
            ...
            if ($user != "") {
                if (Hash::check($request->password, $user->password)) {
                    ...
                    if ($role_id == 2) {
                        ...
                        $childrens = SmStudent::with(['school:id,school_name', 'class:id,class_name', 'section:id,section_name'])->where('parent_id', $parent->id)->where('active_status', 1)->get();
                        $data['userDetails'] = $parent;
                        $data['childrens'] = $childrens;
                        //SmParent::where('user_id', $user->id)->first();
                    } else {
                        $data['userDetails'] = SmStaff::with(['staff_user.school'])->where('user_id', $user->id)->first();
                    }
    ...
    public function saveUploadContents(Request $request)
    ...
    public function searchStudent(Request $request)
    {
        ...
        $students = '';
        $msg = '';
        //dd(!empty($request->class) && !empty($request->section));
