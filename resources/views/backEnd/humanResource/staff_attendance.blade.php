...
<section class="admin-visitor-area">
    <div class="container-fluid p-0">
            <div class="row">
                ...
                <button id="fetch_from_machine"class="primary-btn small fix-gr-bg">
                    <span class="ti-fetch pr-2"></span>
                    Fetch from machine
                </button>
            </div>
            </div>
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <div class="d-flex radio-btn-flex">
                                                <div class="mr-20">
                                                    <input type="radio" name="attendance[{{$staff->id}}]" id="attendanceP{{$staff->id}}" value="P" class="common-radio attendanceP attendance_type_staff" >
                                                ...
                                                <div class="mr-20">
                                                    <input type="radio" name="attendance[{{$staff->id}}]" id="attendanceA{{$staff->id}}" value="A" class="common-radio attendance_type_staff" checked>
...
<script>
    ...
    $('#fetch_from_machine').click(()=>{
        let url='/sikkha/fetch-from-machine';
        if(window.location.hostname!='localhost'){
          url='/fetch-from-machine'; 
        };
        console.log(url);

       (async () => {
       
        $('#fetch_from_machine').html("Fetching....");
           $('#fetch_from_machine').prop("disabled",true);
  await axios.post(url)
       .then((res)=>{
           ...
           if(data.status){
               $('#fetch_from_machine').html("Fetch from machine");
              $('#fetch_from_machine').prop("disabled",false);
              location.reload();
               ...
               $('#fetch_from_machine').html("Fetch from machine");
              $('#fetch_from_machine').prop("disabled",false);
               toastr.error(data.message, 'something wrong!');
              return;
           }
