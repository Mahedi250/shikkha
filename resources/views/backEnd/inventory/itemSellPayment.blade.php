<div class="container-fluid">
   ...
   <div class="row">
    <div class="col-lg-12">
            ...
            <div class="col-lg-12">
                <div class="input-effect">
                    <select class="niceSelect1 w-100 bb form-control{{ $errors->has('payment_mode') ? ' is-invalid' : '' }}" name="payment_method" id="payment_mode">
                        @if($editData->payment_method==3)
                        <option value="{{@$editData->payment_method}}" selected>{{@$editData->paymentMethodName->method}}</option>
                        @else
        ...
        <div class="row mt-25">
            <div class="col-lg-12" id="add_payment_item_receive_bankAccount">
                <div class="input-effect">
                    <select class="niceSelect1 w-100 bb form-control{{ $errors->has('bank_id') ? ' is-invalid' : '' }}" name="bank_id" id="account_id">
                        @if(isset($editData))
                            <option value="{{$editData->account_id}}" selected>{{@$editData->bankName->bank_name}}</option>
                        @endif
                        </select>
                        <span class="focus-border"></span>
...
<script>
    ...
    $(document).ready(function() {
        $("#add_payment_item_receive_bankAccount").hide();

        $("#payment_mode").on("change", function() {
            if ($(this).val() == "3") {
                $("#add_payment_item_receive_bankAccount").show();
            } else {
                $("#add_payment_item_receive_bankAccount").hide();
            }
        });
