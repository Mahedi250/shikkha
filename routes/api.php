...
Route::get('inserttonode', function () {
    $url = "http://" . $_SERVER['HTTP_HOST'] . "/";
    $all_user = App\User::pluck('username');
    $succes = 0;
    try {
        foreach ($all_user as $value) {
            ...
            curl_setopt_array($curl, array(
                ...
                CURLOPT_POSTFIELDS => array('username' => $value, 'url' => $url),
            ));
...
Route::group(['middleware' => ['XSS', 'auth:api', 'json.response']], function () {
    ...
    //single to node
    Route::get('insert-one/{user}', function ($user) {
        ...
        $url = "http://" . $_SERVER['HTTP_HOST'] . "/";
        if ($user == "") {
            return response()->json("invalid username");
        }
        try {
            $curl = curl_init();
            ...
            curl_close($curl);
            return response()->json($user . " " . "inserted");
        } catch (Exception $e) {
    ...
    Route::post('student-attendance-stores', 'SmStudentAttendanceController@studentAttendanceStore')->name('student-attendance-store')->middleware('userRolePermission:69');
    ...
    Route::post('homework-list', ['as' => 'homework-list', 'uses' => 'api\ApiSmHomeWorkController@searchHomework']);
    ...
    //Teacher Api
    Route::get('get-current-exam', 'SmApiController@getcurrentexam');
    Route::get('get-current-class', 'SmApiController@getcurrentclass');
